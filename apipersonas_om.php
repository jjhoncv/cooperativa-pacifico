...
class ApiPersonas{
    ...
    function getListadoDesemb_pdp(){
        $persona = new Persona();
        $res = $persona->obtListadoDesemb_om();
        $i=1;
        if($res->rowCount()){
	...
	function getImpDesemb_pdp($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->imp_desembolsados_om($funcionario);
	...
	function getTotDesemb_pdp($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->total_desembolsados_om($funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            $tmp = $row['TotalDesem'];
	...
	function funnel_pdp($paso, $funcionario){
		$persona = new Persona();
            ...
            $res = $persona->funnel_pdp_importe_om($paso, $funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            
            $res1 = $persona->funnel_pdp_cantidad_om($paso, $funcionario);
	...
	function actualiza_funcionario_pdp($agente1, $agente2, $agente3){
        $persona = new Persona();
        $res = $persona->update_funcionario_om($agente1, $agente2, $agente3);
    }
    ...
    function getDataPreaprobados_pdp($cant, $funcionario){
        ...
        $preaprob = 0;
            ...
            $res = $persona->obtenerPreAprobados_por_funcionario_om($cant, $funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            $tmp = $row['Total'];
	...
	function consulta_pdp($funcionario){
		$persona = new Persona();
        $res = $persona->activo_funcionario_om($funcionario);
		$row = $res->fetch(PDO::FETCH_ASSOC);
		$tmp = $row['Activo'];	
    ...
    function tarjetasPendientes($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->tarjetasPendientes_persona_om($funcionario, "div1");
            $row = $res->fetch(PDO::FETCH_ASSOC);
			
			$res1 = $persona->tarjetasPendientes_persona_om($funcionario, "div2");
            $row1 = $res1->fetch(PDO::FETCH_ASSOC);
	...
	function getPasos2_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_om($step, $funcionario);
	...
	function getPasos3_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_desemb_om($step, $funcionario);
	...
	function getPasos2_descartados_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_descartados_om($step, $funcionario);
	...
	function getModals_pdp($funcionario){
        $persona = new Persona();
        $res = $persona->obtenerModals_om($funcionario);
        if($res->rowCount()){
            while ($row = $res->fetch(PDO::FETCH_ASSOC)){
				...
				$funcA="";$funcB="";$funcC="";$funcD="";$funcE="";$funcX="";$funcK="";$funcJ="";$correo_f="";$sms_func="";
				if($row['Funcionario']=="Agente1"){
                    $funcX="selected ";
					$correo_f="agente1@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
                if($row['Funcionario']=="Agente2"){
                    $funcK="selected ";
					$correo_f="agente2@om.com";
					$sms_func = " funcionaria asignada es XXXXX XXXX,";
                }
				if($row['Funcionario']=="Agente3"){
                    $funcJ="selected ";
					$correo_f="agente3@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
				if($row['Funcionario']=="Agente4"){
                    $funcA="selected ";
					$correo_f="agente4@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
				if($row['Funcionario']=="Agente5"){
                    $funcB="selected ";
					$correo_f="agente5@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
                if($row['Funcionario']=="Agente6"){
                    $funcC="selected ";
					$correo_f="agente6@om.com";
					$sms_func = " funcionaria asignada es XXXXX XXXX,";
                }
				if($row['Funcionario']=="Agente7"){
                    $funcD="selected ";
					$correo_f="agente7@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
				if($row['Funcionario']=="Agente8"){
                    $funcE="selected ";
					$correo_f="agente8@om.com";
					$sms_func = " funcionario asignado es XXXXX XXXX,";
                }
				...
				echo "<option " . $funcA. "value='Agente4'>[4] Agente</option>". "\n";
				echo "<option " . $funcB. "value='Agente5'>[5] Agente</option>". "\n";
				echo "<option " . $funcC. "value='Agente6'>[6] Agente</option>". "\n";
				echo "<option " . $funcD. "value='Agente7'>[7] Agente</option>". "\n";
				echo "<option " . $funcE. "value='Agente8'>[8] Agente</option>". "\n";
				echo "</select>";
				...
				$convenioX="";$convenio0="";$convenio1="";$convenio2="";$convenio3="";$convenio4="";$convenio5="";$convenio6="";$convenio7="";$convenio8="";$convenio9="";
				if($row['Convenio']==""){
                    $convenioX="selected ";
                }
				if($row['Convenio']=="Convenio0"){
                    $convenio0="selected ";
                }
				...
				if($row['Convenio']=="Convenio6"){
                    $convenio6="selected ";
                }
				if($row['Convenio']=="Convenio7"){
                    $convenio7="selected ";
                }
				if($row['Convenio']=="Convenio8"){
                    $convenio8="selected ";
                }
				if($row['Convenio']=="Convenio9"){
                    $convenio9="selected ";
                }
				
				echo "  <select class='form-select-lg' aria-label='Default select example' id='convenio" .$row['Id']. "'>" . "\n";
                echo "<option " . $convenioX. "value=''>Escoge uno</option>". "\n";
				echo "<option " . $convenio0. "value='Convenio0'>DOMINION PERU SOLUCIONES Y SERVICIOS S.A.C</option>". "\n";
                echo "<option " . $convenio1. "value='Convenio1'>NOGASA S.A.C.</option>". "\n";
				echo "<option " . $convenio2. "value='Convenio2'>VISTONY S.A.C (NOGASA S.A.C)</option>". "\n";
				echo "<option " . $convenio3. "value='Convenio3'>ARTESCO S.A.</option>". "\n";
				echo "<option " . $convenio4. "value='Convenio4'>DECO CLEAN EXPRESS S.A.C</option>". "\n";
				echo "<option " . $convenio5. "value='Convenio5'>PRO OUTSOURCING S.A.C</option>". "\n";
				echo "<option " . $convenio6. "value='Convenio6'>PROTEMAX S.R.L.</option>". "\n";
				echo "<option " . $convenio7. "value='Convenio7'>METROCOLOR S.A.</option>". "\n";
				echo "<option " . $convenio8. "value='Convenio8'>COLEGIO SS.CC RECOLETA</option>". "\n";
				echo "<option " . $convenio9. "value='Convenio9'>CENTRO EDUCAT PARTIC MIXTO REINA DEL MUNDO</option>". "\n";
                echo "</select>";
	...
	function actualiza_paso_pdp($id, $paso){
        $persona = new Persona();
        $res = $persona->update_Paso_om($id, $paso);
    }
	
    function actualiza_observacion_pdp($id, $obs, $monto, $paso, $funcionario, $fec_ingreso, $tipo, $convenio){
        $persona = new Persona();
        $res = $persona->update_Obs_om($id, $obs, $monto, $paso, $funcionario, $fec_ingreso, $tipo, $convenio);
    }
