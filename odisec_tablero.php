...
<html>
<head>
  <title>Odisec - Colocaciones</title>
  <meta charset="utf-8">
...
function drop(ev) {
  ...
  if(ev.target.id!=""){
      ...
      xmlhttp.open("GET", "ajax_credimaq?func=1&id="+data+"&paso="+ev.target.id, true);
      xmlhttp.send();
...
function graba(id){
        ...
        xmlhttp2.open("GET", "ajax_credimaq?func=2&id="+id+"&obs="+document.getElementById("text"+id).value+"&monto="+document.getElementById("mont"+id).value+"&paso="+document.getElementById("combo"+id).value+"&funcionario="+document.getElementById("func"+id).value+"&fec_ingreso="+document.getElementById("fec_ingreso"+id).value+"&tipo="+document.getElementById("tipo"+id).value+"&convenio="+document.getElementById("convenio"+id).value, true);
        xmlhttp2.send(); 
        setInterval("location.reload()",1000);
}
...
function sms(celular, nombre, funcionario, tipo){
        ...
        xmlhttp3.open("GET", "ajax_credimaq?func=3&celular="+celular+"&nombre="+nombre+"&tipo="+tipo+"&funcionario="+funcionario);
        xmlhttp3.send(); 
}
...
function agrega(){
    ...
    xmlhttp4.open("GET", "agrega_eval_credimaq?dni="+dni+"&celular="+celular+"&correo="+correo+"&sueldo="+sueldo+"&funcionario="+funcionario);
    xmlhttp4.send(); 
    setInterval("location.reload()",1000);
}
...
<div id="div2" ondrop="drop(event)" ondragover="allowDrop(event)" class="p-0 custom-scrollbar-css"><div class="card-header bg-secondary text-white">Envio Docs <button type='button' class='btn btn-warning btn-xs' onclick="location.href='odisec_info'">+ Info</button></div>
    ...
    <div class='modal-dialog modal-dialog-centered' role='document'>
        ...
        <div class='modal-body'>
                    ...
                    <select class='form-select-lg' aria-label='.form-select-sm example' id='funcionario'>
						...
						<option value='Agente9'>[9] Agente</option>
...
<body>
<br>
<form action="login_pdp2?pagina=odisec_tablero" method="post" name="cuerpo" autocomplete="off">
