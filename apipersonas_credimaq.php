...
class ApiPersonas{
    ...
    function getListadoDesemb_pdp(){
        $persona = new Persona();
        $res = $persona->obtListadoDesemb_credimaq();
        $i=1;
        if($res->rowCount()){
	...
	function getImpDesemb_pdp($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->imp_desembolsados_credimaq($funcionario);
	...
	function getTotDesemb_pdp($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->total_desembolsados_credimaq($funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            $tmp = $row['TotalDesem'];
	...
	function funnel_pdp($paso, $funcionario){
		$persona = new Persona();
            ...
            $res = $persona->funnel_pdp_importe_credimaq($paso, $funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            
            $res1 = $persona->funnel_pdp_cantidad_credimaq($paso, $funcionario);
	...
	function actualiza_funcionario_pdp($agente1, $agente2, $agente3){
        $persona = new Persona();
        $res = $persona->update_funcionario_credimaq($agente1, $agente2, $agente3);
    }
    ...
    function getDataPreaprobados_pdp($cant, $funcionario){
        ...
        $preaprob = 0;
            ...
            $res = $persona->obtenerPreAprobados_por_funcionario_credimaq($cant, $funcionario);
            $row = $res->fetch(PDO::FETCH_ASSOC);
            $tmp = $row['Total'];
	...
	function consulta_pdp($funcionario){
		$persona = new Persona();
        $res = $persona->activo_funcionario_credimaq($funcionario);
		$row = $res->fetch(PDO::FETCH_ASSOC);
		$tmp = $row['Activo'];	
    ...
    function tarjetasPendientes($funcionario){
        $persona = new Persona();
            ...
            $res = $persona->tarjetasPendientes_persona_credimaq($funcionario, "div1");
            $row = $res->fetch(PDO::FETCH_ASSOC);
			
			$res1 = $persona->tarjetasPendientes_persona_credimaq($funcionario, "div2");
            $row1 = $res1->fetch(PDO::FETCH_ASSOC);
    ...
    function add_pdp($item){
        ...
        $res = $persona->nuevaPersona_credimaq($item);
        //$this->exito('Se ingreso el registro de manera correcta');
    }

	function getAll_pdp($keyword){
        $persona = new Persona();
        $res = $persona->obtenerPersonas_credimaq($keyword);
	...
	function getPasos2_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_credimaq($step, $funcionario);
	...
	function getPasos3_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_desemb_credimaq($step, $funcionario);
	...
	function getPasos2_descartados_pdp($step, $funcionario){
        $persona = new Persona();
        $res = $persona->obtenerPasos2_descartados_credimaq($step, $funcionario);
	...
	function getModals_pdp($funcionario){
        $persona = new Persona();
        $res = $persona->obtenerModals_credimaq($funcionario);
        if($res->rowCount()){
            while ($row = $res->fetch(PDO::FETCH_ASSOC)){
	...
	function actualiza_paso_pdp($id, $paso){
        $persona = new Persona();
        $res = $persona->update_Paso_credimaq($id, $paso);
    }
	
    function actualiza_observacion_pdp($id, $obs, $monto, $paso, $funcionario, $fec_ingreso, $tipo, $convenio){
        $persona = new Persona();
        $res = $persona->update_Obs_credimaq($id, $obs, $monto, $paso, $funcionario, $fec_ingreso, $tipo, $convenio);
    }
