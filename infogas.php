...
include_once 'apipersonas_infogas.php';
	...
	$_SESSION["repetido_infogas"] = "vacio";
	$api->repetidoDni_api_infogas($dni);
	
	if($_SESSION["repetido_infogas"]=="true")
	{
	...
	*/
	...
	if($taxista=="No" or $gnv=="No"){
		$estado = "NO CALIFICA";
		$observaciones = "No es taxista o no tiene GNV";
		$encontrado = "F";
	}
	
	if($Nom==""){
		$estado = "NO CALIFICA";
		$observaciones = "DNI no existente";
		$encontrado = "F";
	}
	...
	$item = array(
				...
				'celular' => $celular,
				'situacion' => $cal_tmp,
				'ingreso_diario' => $sueldo,
				'prestamo_vigente' => $prestamovigente,
			...
			if($encontrado=="V"){
				...
				if($estado=="PRE-APROBADO"){
					...
					$url = "https://cooperativapacifico.online/infogas_tablero";
					$titulo = "[" . $dni . "] - " . $nombre_largo;
					
					//$new ->sendPost_email_infogas($correo, $url, $titulo);
					header("Location: https://cp.com.pe/pacifico/prestamo-taxistas-3/");
				}
				else {
					header("Location: https://cp.com.pe/pacifico/prestamo-taxistas-2/");
				}	
			}
			else{
				header("Location: https://cp.com.pe/pacifico/prestamo-taxistas-2/");
